...
function ulx.zsbotsay( calling_ply, target_plys, message, say_team )
	for i, ply in ipairs(target_plys) do
		if ply:IsBot() and ply.IsZSBot2 then
			if say_team then
	...
	for i, ply2 in ipairs(target_plys) do
		if !ply2:IsBot() or !ply2.IsZSBot2 then
			table.RemoveByValue(target_plys, ply2)
		end
	end
